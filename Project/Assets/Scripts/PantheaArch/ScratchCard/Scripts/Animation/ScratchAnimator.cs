...
public class ScratchAnimator : MonoBehaviour
{
    ...
    private AutoResetUniTaskCompletionSource playCompletionSource;
    ...
    private void Update()
    {
        ...
        {
            UpdateScratches();
            totalTime += Time.deltaTime * AnimationSpeed;
        }
    }
    ...
    private void UpdateScratches()
    {
        ...
        if (progress >= 1f)
        {
            ...
            if (currentScratchIndex == ScratchAnimation.Scratches.Count)
            {
                Stop();
                OnCompleted?.Invoke();
                playCompletionSource?.TrySetResult();
            }
            else
            {
                UpdateScratches();
    ...
    {
        playCompletionSource = AutoResetUniTaskCompletionSource.Create();
        Play();
    ...
    public void Stop()
    {
        ...
        previousPosition = null;
        OnStop?.Invoke();
        playCompletionSource?.TrySetCanceled();
    }
}