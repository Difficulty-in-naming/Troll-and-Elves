...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2DShake : BasePC2D
    {
        ...
        // 修改震动位置计算方法，添加边界检查
        Vector3 ClampShakePosition(Vector3 shakePosition)
        {
            if (_numericBoundaries == null || !_numericBoundaries.UseNumericBoundaries)
            ...
            var currentCameraPosition = ProCamera2D.LocalPosition;

            Vector3 clampedPosition = shakePosition;

            // 检查并限制水平方向
            if (_numericBoundaries.UseLeftBoundary)
            {
                float minX = _numericBoundaries.LeftBoundary + halfScreenWidth;
                if (currentCameraPosition.x + shakePosition.x < minX)
                {
                    clampedPosition.x = minX - currentCameraPosition.x;
                }
            }

            if (_numericBoundaries.UseRightBoundary)
            {
                float maxX = _numericBoundaries.RightBoundary - halfScreenWidth;
                if (currentCameraPosition.x + shakePosition.x > maxX)
                {
                    clampedPosition.x = maxX - currentCameraPosition.x;
                }
            }

            // 检查并限制垂直方向
            if (_numericBoundaries.UseBottomBoundary)
            {
                float minY = _numericBoundaries.BottomBoundary + halfScreenHeight;
                if (currentCameraPosition.y + shakePosition.y < minY)
                {
                    clampedPosition.y = minY - currentCameraPosition.y;
                }
            }

            if (_numericBoundaries.UseTopBoundary)
            {
                float maxY = _numericBoundaries.TopBoundary - halfScreenHeight;
                if (currentCameraPosition.y + shakePosition.y > maxY)
                {
                    clampedPosition.y = maxY - currentCameraPosition.y;
                }
            }
        ...
        IEnumerator ShakeRoutine(float smoothness, bool ignoreTimeScale = false)
        {
                   ...
                   Quaternion.Angle(_transform.localRotation, _originalRotation) > .01f)
            {
                ...
                // 应用边界限制
                newShakePosition = ClampShakePosition(newShakePosition);
                ...
                // 再次确保平滑后的位置也在边界内
                newShakePositionSmoothed = ClampShakePosition(newShakePositionSmoothed);
        ...
        IEnumerator ConstantShakeRoutine(float intensity)
        {
            while (_isConstantShaking)
            {
                if (ProCamera2D.DeltaTime > 0)
                {
                    ...
                    _constantShakePosition.Set(
                        Utils.SmoothApproach(_constantShakePosition.x, result.x, result.x, intensity, ProCamera2D.DeltaTime),
                    ...
                    // 应用边界限制
                    _constantShakePosition = ClampShakePosition(_constantShakePosition);
